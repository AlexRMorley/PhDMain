...
SIDEBAR_WIDTH = 260
MAX_BATTERY   = 1000
# ── Charging subsystem config (fork) ──
PER_TYPE_BATTERY = {"Drone": 400}   # per-type max battery; others use MAX_BATTERY
CHARGE_MARGIN    = 45      # spare battery beyond bare travel cost before recall
CHARGE_RATE      = 30.0    # battery refilled per tick while docked
STATION_CAPACITY = 4       # slots per station (rivalry / exclusion -> private good)
STATION_CONG_W   = 10.0    # congestion weight in the station-choice potential game
...
# ── Roles ─────────────────────────────────────────────────────────────────────
class Role(Enum):
    SCOUT = 1; SCAN = 2; LOITER = 3; RELAY = 4; CHARGE = 5
...
class Robot:
    def __init__(self, name, x, y, caps, world, sim,
        ...
        # state
        self.active       = True
        # Per-type battery (drones reduced to 250 in this fork) and charging state.
        self.max_battery  = PER_TYPE_BATTERY.get(robot_type(name), MAX_BATTERY)
        self.battery      = self.max_battery
        self.home         = (x, y)     # spawn cell — fallback recall point
        self.charge_target = None      # station index this robot is recalling to
        self.charging      = False     # True while docked and refilling
    ...
    def move_step(self, occupied: set) -> bool:
        ...
        # ── battery drain ──
        role_mult = {Role.SCOUT:1.4, Role.SCAN:1.0, Role.RELAY:1.1,
                     Role.LOITER:0.6, Role.CHARGE:1.0}[self.role]
...
class FleetSim:
    def __init__(self):
        ...
        self._build_charge_stations()   # fork: charging stations exist from tick 0
    ...
    _W = {Role.SCOUT: 3.0, Role.SCAN: 1.2, Role.LOITER: 0.1, Role.CHARGE: 0.0}
    ...
    _GAMMA = {Role.SCOUT: 1.5, Role.SCAN: 0.5, Role.LOITER: 0.4, Role.RELAY: 0.8, Role.CHARGE: 0.0}
    ...
    def _role_utility_pg(self, robot, role, active, zone, cluster_info):
        ...
        if role == Role.RELAY:
            ...
            uncovered_unclaimed = [
                cl for _, (cl, covered) in cluster_info.items()
                if cl and not covered
                and self._shadow_zone_type.get(cl[0], 'none') in ('stair', 'disc')
                and float(np.mean([self._zone_uf_cache.get(z, 0.0) for z in cl])) > 0.05
                and not any(rr.role == Role.RELAY and rr.task_zone in cl for rr in active)
            ]
            if not uncovered_unclaimed:
                return -5.0   # every needy building already has a relay assigned
            ...
            if is_land_only:
                # Rovers are the ideal relay type — they cannot enter buildings so
                # relay duty costs them almost nothing in exploration value.
                # Scale with demand: more uncovered clusters = higher preference.
                # Also scale with rover scarcity relative to clusters needing relays.
                n_uncovered = sum(1 for _, (cl, cov) in cluster_info.items()
                                  if not cov and cl
                                  and self._shadow_zone_type.get(cl[0], 'none') != 'none')
                n_rovers_free = sum(1 for rr in active
                                    if rr is not robot
                                    and bool(rr.caps_mask & CAP_LAND)
                                    and not bool(rr.caps_mask & (CAP_STAIRS|CAP_AIR|CAP_WATER))
                                    and rr.role != Role.RELAY)
                demand_scale = min(2.0, n_uncovered / max(1, n_rovers_free + 1))
                type_preference = +4.0 * demand_scale
                ...
                # Penalty for stair robots becoming relays:
                # - Base penalty scales with how much building work remains
                # - Amplified when few land-only robots are available to relay instead
                # - Does NOT use "scarcity of stair robots" — even with 6 free stair
                #   robots, each one relaying instead of exploring is wasteful when
                #   buildings are unfinished.
                available_land_only = sum(
                    1 for rr in active
                    if rr is not robot
                    and bool(rr.caps_mask & CAP_LAND)
                    and not bool(rr.caps_mask & (CAP_STAIRS|CAP_AIR|CAP_WATER))
                ...
                n_clusters = max(1, len(cluster_info))
                # How well covered are the relay needs by land-only robots?
                # 0.0 = no land-only robots available (stair robot MUST relay or nobody will)
                # 1.0 = enough land-only robots for all clusters
                land_coverage = min(1.0, available_land_only / n_clusters)
                # Penalty: strong when buildings unfinished AND land robots available
                # Weak only when NO land robot can take over (last resort allowance)
                type_preference = -(4.0 * unexplored_stair_frac * land_coverage
                                    + 1.5 * unexplored_stair_frac * (1.0 - land_coverage))
    ...
    def _pg_best_response_roles(self, active, clusters):
        ...
        eligible = [
            ...
            and r.role != Role.CHARGE        # charging robots are out of the coverage game
    ...
    # ── relay anchor logic ────────────────────────────────────────────────────
    # ───────────────────────── CHARGING SUBSYSTEM (fork) ─────────────────────
    # Charging stations are a PRIVATE GOOD: rivalrous (a slot in use is
    # unavailable to others) and excludable (finite capacity locks others out) —
    # the opposite of the relay's non-rivalrous public-good coverage.
    #
    # The decision to RECALL is a SURVIVAL OVERRIDE, not a strategic role choice:
    # the death payoff dominates everything, so a robot recalls the moment its
    # battery can only just still reach a station.  While CHARGE it is removed
    # from the coverage game's player set (excluded in _pg_best_response_roles).
    #
    # WHICH station it goes to IS a game — a congestion game (Rosenthal 1973):
    # each station is a resource whose cost rises with its load, so robots
    # best-respond to a min-cost station and distribute across stations at a
    # pure-strategy Nash equilibrium.  Congestion games are exact-potential
    # games, so this stays inside the potential-game family even though the
    # resource itself flipped from public good to private/rivalrous.

    def _build_charge_stations(self):
        """Place stations: 3 corners + 1 central river station.

        With drones at 250 battery (2.0 drain/move) a drone can only travel ~125
        cells one-way, so 4 CORNER stations leave the map centre (~196 Manhattan
        from any corner) as an unreachable death zone.  The 4th station is
        therefore placed on the river as close to the map centre as possible, so
        the whole map is within return range of *some* station.  Drones (AIR)
        reach any station; the central one is also water/bridge so a Boat could
        use it too if you later extend charging to other types."""
        W, H = self.world.w, self.world.h
        corners = [(2, 2), (W - 3, 2), (2, H - 3)]
        # 4th station: the water/bridge cell nearest the map centre (the river
        # runs diagonally through it); fall back to nearest free cell to centre.
        cx, cy = W // 2, H // 2
        best_water = best_free = None
        bw = bf = float("inf")
        for x in range(W):
            for y in range(H):
                t = self.world.grid[x][y]["t"]
                d = abs(x - cx) + abs(y - cy)
                if t in (T_WATER, T_BRIDGE) and d < bw:
                    bw, best_water = d, (x, y)
                elif t == T_FREE and d < bf:
                    bf, best_free = d, (x, y)
        central = best_water if best_water is not None else (best_free or (cx, cy))
        pts = corners + [central]
        self.charge_stations = [
            {"pos": p, "cap": STATION_CAPACITY, "occupants": set()} for p in pts
        ]

    @staticmethod
    def _drain_rate(r):
        return {"Legged": 1.0, "Drone": 2.0, "Boat": 2.0, "Rover": 0.4}.get(
            robot_type(r.name), 1.0)

    def _station_load(self, idx, exclude=None):
        """Rivalrous load = docked + en-route claimants. Drives congestion cost."""
        st = self.charge_stations[idx]
        enroute = sum(
            1 for rr in self.robots
            if rr.active and rr is not exclude and rr.role == Role.CHARGE
            and getattr(rr, "charge_target", None) == idx
            and rr.name not in st["occupants"]
        )
        return len(st["occupants"]) + enroute

    def _choose_station(self, r):
        """Congestion game: cost(s) = travel*drain + STATION_CONG_W*load(s).
        Robot best-responds to the min-cost station.  Iterated over the fleet
        this is best-response dynamics on a Rosenthal congestion game, whose
        exact potential Phi = sum_s sum_{k=1..n_s} c_s(k) guarantees convergence
        to a pure Nash equilibrium of station assignments."""
        drain = self._drain_rate(r)
        best_idx, best_cost = None, float("inf")
        for idx, st in enumerate(self.charge_stations):
            d = abs(r.pos[0] - st["pos"][0]) + abs(r.pos[1] - st["pos"][1])
            cost = d * drain + STATION_CONG_W * self._station_load(idx, exclude=r)
            if cost < best_cost:
                best_cost, best_idx = cost, idx
        return best_idx

    def _charge_management(self):
        """Survival-override entry/exit of the CHARGE state + station re-selection.
        Runs every tick right after _decide_roles so it can preempt any coverage
        role assigned to a low-battery robot."""
        if not hasattr(self, "charge_stations"):
            self._build_charge_stations()

        for r in self.robots:
            if not r.active:
                continue
            # death penalty — what makes CHARGE essential
            if r.battery <= 0:
                r.active = False
                r.death_reason = "battery depleted"
                self.dead_robots.append((r.name, r.death_reason))
                idx = getattr(r, "charge_target", None)
                if idx is not None:
                    self.charge_stations[idx]["occupants"].discard(r.name)
                continue

            drain = self._drain_rate(r)

            # ── already docked: refill; release the (rivalrous) slot when full ──
            if r.role == Role.CHARGE and r.charging:
                r.battery = min(r.max_battery, r.battery + CHARGE_RATE)
                if r.battery >= r.max_battery:
                    idx = r.charge_target
                    if idx is not None:
                        self.charge_stations[idx]["occupants"].discard(r.name)
                    r.charging = False
                    r.charge_target = None
                    r.role = Role.SCAN
                    r.role_locked_until = self.timestep   # free to re-role next tick
                continue

            # ── survival override: recall iff battery only just covers the trip ──
            if r.role != Role.CHARGE:
                idx = self._choose_station(r)
                if idx is not None:
                    st = self.charge_stations[idx]
                    d = abs(r.pos[0] - st["pos"][0]) + abs(r.pos[1] - st["pos"][1])
                    need = d * drain + CHARGE_MARGIN
                    if r.battery <= need:
                        r.role = Role.CHARGE
                        r.charge_target = idx
                        r.charging = False
                        # long lock: the coverage role game won't touch it while charging
                        r.role_locked_until = self.timestep + 100000
            else:
                # En route and not yet docked.  Keep the committed target STICKY:
                # re-running the congestion game every tick made drones oscillate
                # between stations and burn the battery they needed to arrive.
                # Only re-pick if we somehow lost the target, or it is now full
                # while we are still far from it (so we don't arrive to no slot).
                if r.charge_target is None:
                    r.charge_target = self._choose_station(r)
                else:
                    st = self.charge_stations[r.charge_target]
                    d = abs(r.pos[0]-st["pos"][0]) + abs(r.pos[1]-st["pos"][1])
                    if d > 3 and len(st["occupants"]) >= st["cap"]:
                        alt = self._choose_station(r)
                        if alt is not None:
                            r.charge_target = alt

    def _move_to_charge(self, r, occupied):
        """Move a CHARGE robot one step toward its chosen station; dock on arrival.
        Station capacity is enforced here (rivalry/exclusion): if the station is
        full on arrival, the robot re-enters the congestion game next tick."""
        idx = getattr(r, "charge_target", None)
        if idx is None:
            r.charge_target = idx = self._choose_station(r)
        if idx is None:
            return
        st = self.charge_stations[idx]
        spos = st["pos"]

        # arrived (on or adjacent to the station cell)
        if abs(r.pos[0] - spos[0]) + abs(r.pos[1] - spos[1]) <= 1:
            if r.name in st["occupants"]:
                r.charging = True; return
            if len(st["occupants"]) < st["cap"]:
                occupied.discard(r.pos)
                st["occupants"].add(r.name)
                r.charging = True
                r.pos = spos
                occupied.add(spos)
                return
            # full — congestion realised: pick a less-loaded station next tick
            r.charge_target = self._choose_station(r)
            return

        # navigate one shadow-free step toward the station (minimal-config A*)
        tb_arr = r.terrain_belief
        shadow = self.radio_shadow
        _zero_traffic = np.zeros((GRID_W, GRID_H), dtype=np.uint16)
        _zero_chunked = np.zeros((2, GRID_W // CHUNK_SIZE, GRID_H // CHUNK_SIZE),
                                 dtype=np.float32)
        need_replan = (r.goal != spos) or (not r.path)
        if need_replan:
            path = AStar.search(
                start=r.pos, goal=spos,
                ...
                radio_shadow=shadow, relay_ok_fn=lambda z: True,
            ...
            if not path:
                return   # can't path right now; try again next tick
            r.goal = spos; r.path = path

        if r.path:
            next_cell = r.path[0]
            true_t = self.world.grid[next_cell[0]][next_cell[1]]["t"]
            # respect terrain feasibility for non-air robots
            if true_t == T_OBS:
                r.path = []; return
            if true_t == T_WATER and not bool(r.caps_mask & (CAP_WATER | CAP_AIR)):
                r.path = []; r.goal = None; return
            if (true_t in (T_FREE, T_STAIRS) and bool(r.caps_mask & CAP_WATER)
                    and not bool(r.caps_mask & CAP_AIR)):
                r.path = []; r.goal = None; return
            occupied.discard(r.pos); occupied.add(next_cell)
            r.pos = r.path.pop(0)
            if r._reveal_all():
                r._scan_dirty = True
            r.battery -= self._drain_rate(r) * 1.0
    ...
    def step(self) -> bool:
        ...
        # ── CHARGE override: survival-triggered recall preempts coverage roles ──
        self._charge_management()
        ...
        for r in self.robots:
            ...
            elif r.role == Role.CHARGE:
                self._move_to_charge(r, occupied)
        ...
        for r in self.robots:
            if not r.active: continue
            if r.role in (Role.RELAY, Role.CHARGE): continue
            if r.battery <= 0:
                if r.active:
