...
SHADOW_ZONE_DONE = 0.85   # shadow zones: exit at 85% — last ~15% not worth
                           # keeping a relay running and explorer inside for
...
class Robot:
    ...
    def _process_inbox(self):
        """Merge received messages into local belief using vectorised ops."""
        if not self.inbox: return
        now = self.sim.timestep
        # Unpack all messages into arrays for vectorised processing
        xs   = np.array([m['x']       for m in self.inbox], dtype=np.int16)
        ys   = np.array([m['y']       for m in self.inbox], dtype=np.int16)
        ts   = np.array([m['ts']      for m in self.inbox], dtype=np.int32)
        terr = np.array([m['terrain'] for m in self.inbox], dtype=np.uint8)
        temp = np.array([m['temp']    for m in self.inbox], dtype=np.float32)
        rad  = np.array([m['rad']     for m in self.inbox], dtype=np.float32)
...
class FleetSim:
    ...
    def _assign_zones_cbba(self):
        ...
        # Relay robots always get their bundle cleared — they don't explore zones.
        for r in self.robots:
            if not r.active or r.battery <= 0: continue
            if r.role == Role.RELAY:
                # Release any zones this relay was holding back to free
                for z in r.bundle:
        ...
        # refresh zone task progress — shadow zones release at lower threshold
        for z, task in self.zone_tasks.items():
            task.progress = self.zone_coverage(self.union_belief, z)
            is_shz = self._shadow_frac_for_zone(z) > 0.2
        ...
        for _it in range(CBBA_ITERS):
            ...
            for r in self.robots:
                if not r.active or r.battery <= 0: continue
                # Relay robots don't explore zones — skip bidding entirely.
                # Previously relay bundles were filled by CBBA then immediately
                # cleared at election, but CBBA would refill them the next tick,
                # blocking 3-4 zones per relay from being assigned to explorers.
                if r.role == Role.RELAY:
                    continue
                bundle_counts[r.name] = len(r.bundle)
                bids = []
                # Cache other_capable count for battery urgency — O(R) once per robot
                # instead of O(R) inside _zone_utility called O(Z) times = O(R×Z) total
                _other_capable = sum(
        ...
        for r in self.robots:
            if not r.active or r.battery <= 0: continue
            if r.role == Role.RELAY: continue  # relays don't explore zones
            if not r.bundle:
                z = self._fallback_zone(r)
                if z: r.bundle.append(z); r.assigned_zones.append(z)

        # ── Stair-zone rescue ───────────────────────────────────────────────────
        # CBBA greedy bundle selection fills slots with nearby zones first, leaving
        # distant relay-covered stair zones (high uf, no owners) perpetually
        # unassigned. Force-assign each such zone to the nearest capable robot that
        # has a free bundle slot.
    ...
    def _zone_utility(self, r, zone, bundle_counts, other_capable_cache=None):
        ...
        if personal_frac >= ZONE_PERSONAL_THRESH:
            ...
            if relay_active and zone_type == 'stair' and can_enter_shadow:
                ...
                relay_explorer_bonus = 6.0 * sf * max(uf, 0.5)  # disc open to all
            else:
    ...
    def _decide_roles(self):
        ...
        # ── 1. Build shadow clusters using cell-level adjacency (_shadow_zone_adj) ─
        # This is the same graph as _compute_relay_flood and _same_shadow_cluster,
        # so disc and stair zones form separate clusters. The old shadow_frac>0.05
        # BFS merged them — disc interiors ended up inside stair clusters and never
        # got their own relay elected.
        shadow_zones = [
            (zx, zy)
            for zx in range(self.zone_nx)
            for zy in range(self.zone_ny)
    ...
    # Actual contribution is scaled by capability_yield — making this type-aware.
    _W = {Role.SCOUT: 2.4, Role.SCAN: 1.2, Role.LOITER: 0.1}

    # Congestion coefficients γ(a): cost of each additional robot choosing role a.
    _GAMMA = {Role.SCOUT: 0.8, Role.SCAN: 0.5, Role.LOITER: 0.4, Role.RELAY: 0.8}
    ...
    def _mean_field_signal(self, robot, zone) -> float:
        """
        Mean field approximation for inter-zone coordination at fleet scale.

        Rather than reasoning about specific robots elsewhere on the map (O(R²)),
        robots reason about the aggregate distribution: is this zone type currently
        underserved by capable robots relative to the number of zones needing them?

        This is O(Z) — iterates over zones not robots — and provides the
        fleet-wide rebalancing signal that prevents capable robots from clustering
        in one quadrant while buildings on the other side go unexplored.

        Returns a bonus [0, ~1.5] added to effort utility for zones where
        this robot's capability type is under-represented fleet-wide.
        """
        if zone is None:
            return 0.0
        ...
        # Count capable robots already assigned to zones of this type
        ...
        # Count zones of this type still needing exploration and accessible
        n_zones_needing = sum(
            1 for z2, zt2 in self._shadow_zone_type.items()
            if zt2 == zt
            and self.zone_stats(z2)['unknown_frac'] > 0.10
            and (zt2 == 'none' or self._relay_ok_flood.get(z2, False))
        )
        if n_zones_needing == 0:
            return 0.0

        # Underservice ratio: zones needing coverage vs capable robots assigned
    ...
    def _relay_val(self, cluster):
        ...
        Cached per cluster-key per tick — called up to 71x/step in BR loop.
        """
        # Tick-level cache keyed by frozenset of cluster zones
        ...
        if cluster_type == 'stair':
            # Vectorised: use precomputed world stair array and union_belief slice
            # instead of nested Python loop over individual cells (was 188k calls/20 steps)
            unknown_stair = 0
            for z in cluster:
                zx, zy = z
                x0 = zx*self.zone_w_cells; x1 = min(x0+self.zone_w_cells, self.world.w)
                y0 = zy*self.zone_h_cells; y1 = min(y0+self.zone_h_cells, self.world.h)
                stair_mask = self._world_stair_arr[x0:x1, y0:y1]
                unk_mask   = (self.union_belief[x0:x1, y0:y1] == T_UNKNOWN)
                unknown_stair += int(np.count_nonzero(stair_mask & unk_mask))
            cluster_uf_stair = float(np.mean([self.zone_stats(z)['unknown_frac']
                                              for z in cluster])) if cluster else 0.0
            # Proximity demand: capable explorer heading here soon
            explorer_demand = False
            cx_c = int(np.mean([z[0]*self.zone_w_cells + self.zone_w_cells//2 for z in cluster]))
            cy_c = int(np.mean([z[1]*self.zone_h_cells + self.zone_h_cells//2 for z in cluster]))
            for r in self.robots:
                if not r.active or r.role == Role.RELAY: continue
                if not bool(r.caps_mask & (CAP_STAIRS | CAP_AIR)): continue
                if r.task_zone not in cluster_zones: continue
                dist = abs(r.pos[0] - cx_c) + abs(r.pos[1] - cy_c)
                assigned = r.task_zone in cluster_zones
                if dist < 40 or (dist < 70 and assigned):
                    explorer_demand = True; break
            # Full relay value when: explorer nearby, OR cluster partially explored
            # (0.15 < uf < 0.70 means someone has been inside — safety critical).
            # Fully unknown clusters (uf≈1.0) haven't been entered yet so no urgency.
            if explorer_demand or (cluster_uf_stair > 0.15):
                stair_bonus = 2.0 + unknown_stair / max(1, zone_size)
                val = max(val, stair_bonus)
            else:
                val = max(val, 0.3)
        elif cluster_type == 'disc':
            val *= 2.5   # disc shadow is large; boost so it competes with stair
    ...
    def _role_utility_pg(self, robot, role, active, zone, cluster_info):
        ...
        """
        if role == Role.RELAY:
            # Relay utility unchanged from original — already robot-specific
            # via distance and capability checks in the BR loop.
        ...
        for _iter in range(max_iters):
            ...
            for robot in eligible:
                ...
                if best_role == Role.RELAY:
                    # Cap: demand-driven but with a survivor-safety override.
                    # A cluster counts as having demand if ANY of:
                    #   (a) A capable explorer is within 70 cells AND assigned, OR
                    #   (b) A capable explorer is within 40 cells (about to arrive), OR
                    #   (c) Cluster has meaningful unknown area (uf > 0.15) — this
                    #       covers both completely unvisited buildings (uf≈1.0, need
                    #       a relay to become accessible at all) AND partially explored
                    #       buildings (0.15 < uf < 0.70, mid-exploration safety).
                    #       The old window 0.15 < uf < 0.70 accidentally excluded fully
                    #       unknown buildings, leaving them permanently inaccessible.
                    clusters_with_demand = 0
                    for cid2, (cl2, _) in cluster_info.items():
                        if not cl2: continue
                        ct2 = self._shadow_zone_type.get(cl2[0], 'none')
                        if ct2 not in ('stair', 'disc'): continue
                        cx2 = int(sum(z[0]*self.zone_w_cells + self.zone_w_cells//2 for z in cl2) / len(cl2))
                        cy2 = int(sum(z[1]*self.zone_h_cells + self.zone_h_cells//2 for z in cl2) / len(cl2))
                        cluster_uf2 = float(np.mean([self.zone_stats(z)['unknown_frac'] for z in cl2]))
                        # Safety override: any building with significant unknown area
                        # needs a relay — whether it's unvisited or mid-exploration
                        if cluster_uf2 > 0.15:
                            clusters_with_demand += 1; continue
                        # Proximity demand: explorer heading to nearly-done building
                        for rr in active:
                            if not rr.active or rr.role == Role.RELAY: continue
                            if ct2 == 'stair' and not bool(rr.caps_mask & (CAP_STAIRS | CAP_AIR)): continue
                            dist = abs(rr.pos[0]-cx2)+abs(rr.pos[1]-cy2)
                            assigned = rr.task_zone in set(cl2)
                            if dist < 40 or (dist < 70 and assigned):
                                clusters_with_demand += 1; break
                    relay_cap = max(1, min(len(clusters), clusters_with_demand))
                    current_relays = sum(1 for rr in active if rr.role == Role.RELAY)
                    if current_relays >= relay_cap:
                        robot.role = old_role
                    ...
                    if best_cl is not None:
                        ...
                        # Short lock at election — just prevents immediate demotion
                        # while the relay travels to the border. Real hold of
                        # RELAY_MIN_HOLD starts once the relay physically arrives.
                        robot.relay_hold_until   = t + 30   # travel grace period
                        robot.role_locked_until  = t + 30
    ...
    def _move_relay(self, r, occupied):
        ...
        # Demotion — only evaluate when the relay is physically at the border
        # and has been serving (hold >= RELAY_MIN_HOLD means it arrived and started).
        # Once at the border, demote the moment the last explorer exits — do NOT
        # wait for hold_until to expire. The old gate (hold <= timestep) was the
        # bug: auto-extend pushed hold to t_exit+150, locking the relay for 150
        # more ticks after the building was already vacated.
        relay_is_serving = (r.relay_hold_until >= self.timestep + RELAY_MIN_HOLD - 5
                            or r.relay_hold_until >= self.timestep)
        if relay_is_serving and border_mask[r.pos[0], r.pos[1]]:
            ...
            cluster_uf = float(np.mean([self.zone_stats(z)['unknown_frac']
                                        for z in cluster_zones])) if cluster_zones else 0.0
            exp_inside = any(
                rr.active and rr.role != Role.RELAY
                and self.radio_shadow[rr.pos[0], rr.pos[1]]
                and self.cell_to_zone(rr.pos[0], rr.pos[1]) in set(cluster_zones)
                for rr in self.robots
            )
            # Also check if a capable explorer is close and likely to enter soon
            # — don't demote just because the explorer briefly stepped outside.
            explorer_approaching = any(
                rr.active and rr.role != Role.RELAY
                and bool(rr.caps_mask & (CAP_STAIRS | CAP_AIR))
                and rr.task_zone in set(cluster_zones)
                and abs(rr.pos[0]-cx)+abs(rr.pos[1]-cy) < 25
                for rr in self.robots
            )
            full_hold_started = r.relay_hold_until > self.timestep + 30
            # Safety lock: never demote while cluster is partially explored —
            # someone has been inside and may be returning. Fully unknown clusters
            # (uf≈1.0) haven't been entered yet so demotion is fine there.
            safety_lock = cluster_uf > 0.15
            should_demote = (full_hold_started
                             and not exp_inside
                             and not explorer_approaching
                             and not safety_lock
                             and cluster_uf < (1.0 - SHADOW_ZONE_DONE))
            if should_demote:
                r.role = Role.SCAN
        ...
        if border_mask[r.pos[0], r.pos[1]]:
            ...
            if own_cluster_border:
                # On first arrival at border, start the full RELAY_MIN_HOLD clock.
                # This decouples hold time from travel time — short election lock lets
                # the potential game re-evaluate quickly if the relay gets lost, and
                # the full hold only starts once the relay is actually serving.
                if r.relay_hold_until < self.timestep + RELAY_MIN_HOLD:
                    r.relay_hold_until  = self.timestep + RELAY_MIN_HOLD
                    r.role_locked_until = self.timestep + RELAY_MIN_HOLD
                ...
                if explorers_inside:
                    # Extend hold only when it's about to expire (within 20 ticks).
                    # Previously extended every tick an explorer was inside, which
                    # accumulated to hold_until = exit_tick + 150 for each of N entry
                    # ticks — keeping relays frozen long after buildings were cleared.
                    if r.relay_hold_until < self.timestep + 20:
                        r.relay_hold_until  = self.timestep + RELAY_MIN_HOLD
                        r.role_locked_until = self.timestep + RELAY_MIN_HOLD
    ...
    def _choose_goal(self, r) -> tuple | None:
        ...
        if (r.task_zone is not None
            ...
            if zone_cov < SHADOW_ZONE_DONE * 0.85:
                # Zone still substantially unknown — pull INTO shadow
                in_shadow = np.array(
                ...
                # Robot is inside — find wall-adjacent unknown cells (the slither)
                # using vectorised numpy ops instead of a nested Python loop.
                zx2, zy2 = r.task_zone
                ...
                ub_sl  = union[zx0:zx1, zy0:zy1]          # belief slice
                unk_sl = (ub_sl == T_UNKNOWN)              # unknown mask

                # World terrain slice — precomputed array
                ...
                # A cell is wall-adjacent if any 4-neighbour is a wall.
                # Use scipy-style shift: pad then shift slices.
                W2, H2 = wall_sl.shape
                adj_wall = np.zeros((W2, H2), dtype=bool)
                adj_wall[:-1, :] |= wall_sl[1:, :]   # right neighbour is wall
                adj_wall[1:,  :] |= wall_sl[:-1, :]  # left
                adj_wall[:, :-1] |= wall_sl[:, 1:]   # down
                adj_wall[:, 1:]  |= wall_sl[:, :-1]  # up

                wall_adj_mask = unk_sl & adj_wall     # unknown AND wall-adjacent

                if np.any(wall_adj_mask):
                    # Build set of absolute coords for O(1) lookup in candidate loop
                    wy_idx, wx_idx = np.where(wall_adj_mask)  # note: shape is (W2,H2)
                    wall_adj_unk = set(
                        zip((wy_idx + zx0).tolist(), (wx_idx + zy0).tolist()))
                    wall_bonus = np.array(
                        [80.0 if (cx_a[i], cy_a[i]) in wall_adj_unk else 0.0
                         for i in range(N)], dtype=np.float32)
                    shadow_pull = wall_bonus
                else:
                    # All wall cells seen — now bias toward exit
                    in_shadow = np.array(
                        [self.radio_shadow[cx_a[i], cy_a[i]] for i in range(N)],
                        dtype=np.float32)
                    shadow_pull = -in_shadow * 40.0
            else:
                # Robot outside, zone mostly done — bias toward exit
                in_shadow = np.array(
                    [self.radio_shadow[cx_a[i], cy_a[i]] for i in range(N)],
                    dtype=np.float32)
                shadow_pull = -in_shadow * 40.0
    ...
    def _manage_task_zone(self, r):
        ...
        # Shadow zones: half the patience — if no new cells found in 15 ticks,
        # the robot is stuck chasing inaccessible corners. Exit and let others try.
        no_progress_limit = (NO_PROGRESS_K // 2
                             if is_shadow_zone else NO_PROGRESS_K)
        if r.role != Role.RELAY and not lease_active and r.task_no_progress >= no_progress_limit:
    ...
    def _move_robot(self, r, occupied):
        ...
        if need_new:
            ...
            if r.bundle and r.task_zone is None:
                all_exhausted = all(
                    not self.zone_frontiers_for(r, z)
                    for z in r.bundle
