...
class GridWorld:
    ...
    def _place_bridge_on_component(self, comp_cells, bridge_w=5):
        ...
        def is_water(x, y):
            return 0 <= x < self.w and 0 <= y < self.h and self.grid[x][y].true_terrain == Terrain.WATER
        ...
        while True:
            ...
            span.append((nbx, nby))
            bx, by = nbx, nby
            if true_t(bx, by) is not Terrain.WATER: break
        ...
        for (bx, by) in span:
            for j in range(-half_w, half_w + 1):
                ...
                if 0 <= px < self.w and 0 <= py < self.h:
                    self.grid[px][py] = Cell(Terrain.STAIRS)
    ...
    def _stamp_house(self, hx, hy, house_w, house_h):
        ...
        for x in range(hx, hx + house_w):
            self.grid[x][hy] = Cell(Terrain.OBSTACLE)
            self.grid[x][hy + house_h - 1] = Cell(Terrain.OBSTACLE)
        for y in range(hy, hy + house_h):
            self.grid[hx][y] = Cell(Terrain.OBSTACLE)
            self.grid[hx + house_w - 1][y] = Cell(Terrain.OBSTACLE)
        ...
        for dx in range(-(door_w//2), door_w//2 + 1):
            x = door_x + dx
            if hx <= x < hx + house_w:
                self.grid[x][door_y] = Cell(Terrain.STAIRS)     # yellow doorway
                # also ensure the cell just inside is stairs (already is, but safe)
                if door_y - 1 > hy:
                    self.grid[x][door_y - 1] = Cell(Terrain.STAIRS)
    ...
    def _stamp_box(self, bx, by, bw, bh):
        for x in range(bx, bx + bw):
            self.grid[x][by] = Cell(Terrain.OBSTACLE)
            self.grid[x][by + bh - 1] = Cell(Terrain.OBSTACLE)
        for y in range(by, by + bh):
            self.grid[bx][y] = Cell(Terrain.OBSTACLE)
            self.grid[bx + bw - 1][y] = Cell(Terrain.OBSTACLE)

            # door
        door_x = bx + bw // 2
        self.grid[door_x][by] = Cell(Terrain.FREE)
            ...
            for y in range(self.h):
                ...
                while q:
                    ...
                    for nx, ny in ((cx-1,cy),(cx+1,cy),(cx,cy-1),(cx,cy+1)):
                        if 0 <= nx < self.w and 0 <= ny < self.h:
                            if self.grid[nx][ny].true_terrain == Terrain.WATER and (nx, ny) not in visited:
                                visited.add((nx, ny))
                                q.append((nx, ny))
                comps.append(comp)
        return comps
    ...
    def search(self):
        ...
        while open_set:
            ...
            for nbr in self.world.neighbours(current):
                ...
                # Land robots: avoid UNKNOWN cells that are adjacent to known WATER
                if (Capability.LAND in self.caps) and (Capability.AIR not in self.caps) and (Capability.WATER not in self.caps):
                    if tb is Terrain.UNKNOWN:
                        water_adj = any(self.terrain_belief[nn[0], nn[1]] is Terrain.WATER
                                        for nn in self.world.neighbours(nbr))
                        if water_adj:
                            continue


                # Boats: do NOT roam into unknown "land" — only allow unknown if it's adjacent to known water
                if (Capability.WATER in self.caps) and (Capability.AIR not in self.caps):
                    if tb is Terrain.UNKNOWN:
                        water_adj = False
                        for nn in self.world.neighbours(nbr):
                            if self.terrain_belief[nn[0], nn[1]] is Terrain.WATER:
                                water_adj = True
                                break
                        if not water_adj:
                            continue
                ...
                if tb is not Terrain.UNKNOWN:
                    ...
                    # If you want "avoidance weights only" keep max(0,...)
                    wT = max(0.0, float(self.weights[0]))
                    wR = max(0.0, float(self.weights[1]))
                    e  = max(wT * eT, wR * eR)

                    # --- radio shadow feasibility gate ---
                if self.radio_shadow[nbr[0], nbr[1]]:
                    z = self.zone_of_cell(nbr[0], nbr[1])
                    if (z is not None) and (not self.relay_ok.get(z, False)):
                        continue
        ...
        return []
    #Doctoral academy, RTSG just find out how much and how to access
...
class Robot:
            ...
            for dy in range(-R, R+1):
                if dx*dx + dy*dy <= R*R:
                    ...
                    if 0 <= nx < self.world.w and 0 <= ny < self.world.h:
                        ...
                        # KEEP only this terrain assignment (numeric code)
                        self.terrain_belief[nx, ny] = TERR_TO_CODE[cell_true.true_terrain]
    ...
    def step(self):
        ...
        if (Capability.WATER in self.caps) and (Capability.AIR not in self.caps):
            if true_t is not Terrain.WATER:
        ...
        if self.sim.radio_shadow[self.pos[0], self.pos[1]]:
            ...
            if not self.sim.relay_ok.get(z, False):
                self.active = False
                self.goal = None
                self.path = []
...
class FleetSim:
    def __init__(self):
        ...
        water_cells = [
            (x,y) for x in range(self.world.w)
                ...
                if self.world.grid[x][y].true_terrain == Terrain.WATER
        ]
        ...
        for i, tname in enumerate(spawn_list):
            ...
            if tname == "Boat" and water_cells:
                ...
                for _ in range(30):
                    ...
                    if tt == Terrain.FREE or (tt == Terrain.STAIRS and Capability.STAIRS in caps):
                        break
    ...
    def zone_frontiers(self, union, reachable, zone, robot=None):
        ...
        for x in xs:
            for y in ys:
                ...
                if union[x, y] == Terrain.UNKNOWN:
                    out.append((x, y))
                elif robot is not None and robot.name.startswith("Boat") and union[x, y] == Terrain.WATER:
                    # water cell that can reveal unknown neighbors
                    if any(union[nx, ny] == Terrain.UNKNOWN for (nx, ny) in self.world.neighbours((x, y))):
                        out.append((x, y))
        return out
    ...
    def step(self):
        ...
        def process_robot(r):
            ...
            recv = (r.terrain_belief == Terrain.UNKNOWN) & (uTerr != Terrain.UNKNOWN)
            if np.any(recv):
                ...
                while dq:
                    ...
                    for v in self.world.neighbours(u):
                        ...
                        if (Capability.LAND in r.caps) and (Capability.AIR not in r.caps) and (Capability.WATER not in r.caps):
                            if tb is Terrain.UNKNOWN:
                                water_adj = any(r.terrain_belief[nn[0], nn[1]] is Terrain.WATER for nn in self.world.neighbours(v))
                                if water_adj:
                                    continue
            ...
            global_cov = np.mean(union != Terrain.UNKNOWN)
        ...
        for zx in range(self.zone_nx):
            for zy in range(self.zone_ny):
                relay_ok[(zx, zy)] = self.zone_has_outside_relay((zx, zy))
        self.relay_ok = relay_ok
    ...
    def choose_zone_goal(self, r, zone, union, reachable):
        ...
        if r.role == Role.RELAY:
            ...
            if anchors:
                # pick anchor near zone center
                return min(anchors, key=lambda p: heuristic(p, (cx, cy)))
        ...
        if r.role == Role.SCOUT:
            ...
            def scout_score(c):
                unk = sum(1 for (nx, ny) in self.world.neighbours(c) if union[nx, ny] == Terrain.UNKNOWN)
    ...
    def role_utility(self, r: Robot, role: Role, zone, role_counts, stats):
        ...
        if role == Role.RELAY:
            U_info += 3.0 * sf     # bump it; 1.8 is often too weak

        
        relay_count = role_counts.get(Role.RELAY, 0)
        if sf > 0.05 and relay_count == 0 and role in (Role.SCOUT, Role.SCAN):
            U_rule -= 3.0 * sf
    ...
    def build_radio_shadow(self):
        ...
        for x in range(GRID_W):
            for y in range(GRID_H):
                if self.world.grid[x][y].true_terrain == Terrain.STAIRS:
                    rs[x, y] = True
