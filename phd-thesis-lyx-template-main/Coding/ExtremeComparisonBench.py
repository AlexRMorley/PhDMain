"""
Extreme-Environment Five-Way Comparison Benchmark — Coastal Nuclear Industrial Disaster
=======================================================================================
Compares: Fleet | GNF | Greedy RL | CARA-Base | CARA-Dynamic
on the STRUCTURED 200×200 scenario shared with ExtremeEnvGUI.py (single source of
truth in extreme_scenario.py):
  - irregular coastline + sea, meandering river estuary, piers
  - one intact + one collapsed bridge
  - concentric nuclear reactor (single entrance, radiation gradient + SE leak)
  - chemical plant (eastward plume) + tank-farm / warehouse / riverbank / reactor fires
  - residential complex (touching terrace + spaced block), warehouse district, docks
  - a rocky hill (yellow stairs, NOT radio-shadowed)
  - 37 robots, battery 5 000, reveal radius 3
  - 30 survivors distributed 30% open / 40% industrial / 20% reactor / 10% waterfront,
    placed only on survivable + reachable cells

All five methods build the IDENTICAL world / spawns / survivors via the shared
install() (they all delegate world-gen, spawn, shadow and survivors through
M.GridWorld / M.FleetSim / M_gnf.GNFSim), so the comparison is fair by construction.

Usage:
    python ExtremeComparisonBench.py [--steps N] [--seeds 0,1,2] [--out ./results]
"""

import sys, os, random, time, argparse, json, math
...
_SEARCH_DIRS = [_HERE, os.path.join(_HERE, '..', 'outputs'), os.path.join(_HERE, '..'),
...
def _find(*names):
    ...
    for d in _SEARCH_DIRS: print("  " + d)
    sys.exit(1)

def _load(path, name):
    spec = importlib.util.spec_from_file_location(name, path)
    Mx = importlib.util.module_from_spec(spec); spec.loader.exec_module(Mx); return Mx

# Load the SAME fleet module the GUI uses, so the benchmark measures the same Fleet.
M      = _load(_find('2DFleetFrameworkJ.py', 'hetero_robot_fleet_sim.py'), 'fleet')
...
# ── Configuration (matches the GUI demo scenario) ──────────────────────────────
EXT_GRID_W      = 200
EXT_GRID_H      = 200
EXT_CELL_SIZE   = 5
EXT_ZONE_CHUNKS = 10
...
EXT_N_SURVIVORS = 30
EXT_REVEAL_R    = 3
EXT_ROBOTS      = {"Legged": 8, "Drone": 13, "Boat": 6, "Rover": 10}   # 37 total
# (To revert to the lighter sweep, set e.g. {"Legged":5,"Drone":8,"Boat":3,"Rover":5}
#  and EXT_N_SURVIVORS=25 — the shared scenario scales automatically.)

# ── Install the shared structured scenario (patches all sims fairly) ───────────
sys.path.insert(0, _HERE)
import ExtremeScenario as scen
scen.install(M, M_gnf, M_rl,
             grid_w=EXT_GRID_W, grid_h=EXT_GRID_H, cell=EXT_CELL_SIZE,
...
# ── Build sim classes (after install) ──────────────────────────────────────────
FleetSim    = M.FleetSim
GNFSim      = M_gnf.GNFSim
...
try:
    ...
    print("  [cara_sim.py: use_exec_layer not supported — treating both as same]")
...
# ── Metric helpers ─────────────────────────────────────────────────────────────
def _stair_mask(sim):
    """T_STAIRS cells that are actual BUILDINGS (the rocky hill is terrain, not a
    building, so it is excluded — it must not inflate stair/building coverage)."""
    ...
    for (hx, hy) in getattr(sim.world, 'hill_cells', []):
        sm[hx, hy] = False
...
def _zone_redundancy(sim):
    ...
    for r in active:
        z = getattr(r, 'task_zone', None) or sim.cell_to_zone(r.pos[0], r.pos[1])
...
def run_sim(sim_name, factory, seed, steps):
    ...
    for step in range(1, steps + 1):
        ...
        if step % 10 == 0 or step == steps:
            ...
            _progress(step, steps, prefix=f'{sim_name:<14} ',
...
def _mean_std(results_by_name, name, getter):
    """Mean/std of a per-seed time series. Seeds that completed or stalled early
    produce SHORTER series; align them on the longest seed's x-grid and hold each
    short series at its final value (a finished run stays put), so stacking never
    hits a ragged-array error."""
    series = [getter(r) for r in results_by_name[name]]
    series = [s for s in series if s]                 # drop empties
    if not series:
        return np.array([]), np.array([]), np.array([])
    longest = max(series, key=len)                    # x-grids are shared prefixes
    xs = np.array([s for s, _ in longest]); L = len(xs)
    rows = []
    for s in series:
        ys = [v for _, v in s]
        if len(ys) < L:
            ys = ys + [ys[-1]] * (L - len(ys))        # forward-fill (hold last)
        else:
            ys = ys[:L]
        rows.append(ys)
...
def _save(fig, path, name):
    fig.tight_layout(); fig.savefig(os.path.join(path, name), dpi=130); plt.close(fig); return name
...
def main():
    ...
    steps  = args.steps
    out_dir = args.out or os.path.join(_HERE, 'benchmark_extreme_comparison')
    os.makedirs(out_dir, exist_ok=True)

    N_ROBOTS = sum(EXT_ROBOTS.values())
    results_by_name = {name: [] for name, _, _, _ in SIMS}
    accs = {name: defaultdict(float) for name, _, _, _ in SIMS}

    W = 128
    print(f"\n{'=' * W}")
    print(f"  Coastal Nuclear Industrial Disaster — Five-Way Benchmark  —  "
          f"{EXT_GRID_W}×{EXT_GRID_H} grid @ {EXT_CELL_SIZE}m  |  "
          f"{N_ROBOTS} robots  |  {EXT_N_SURVIVORS} survivors")
    print(f"  Scenario: reactor (rad gradient + leak) · chemical plume + fires · "
          f"estuary (1 intact/1 collapsed bridge) · rocky hill (no shadow) · docks")
    print(f"  Survivors: 30% open / 40% industrial / 20% reactor / 10% waterfront "
          f"(survivable + reachable)")
    print(f"  Battery: {EXT_MAX_BATTERY}  |  reveal radius {EXT_REVEAL_R}  |  "
          f"{steps} steps  |  seeds: {seeds}  |  plots -> {out_dir}")
    print(f"{'=' * W}")
    ...
    for seed in seeds:
        ...
        for run_idx, (name, factory, _, _) in enumerate(SIMS):
            ...
            _progress(overall, total_runs, prefix='Overall  ',
            ...
            if is_cara:
                base += (f"  | milp_avg={res.milp_avg:.0f}ms"
                         f"  milp_max={res.milp_max:.0f}ms  n={res.milp_n}"
                         f"  hold={res.hold}  ejects={res.ejects}")
            print(base)
            for k in ('final_cov', 'final_stair', 'final_found', 'deaths', 'hazard_deaths',
                      'battery_deaths', 'trapped', 'p50', 'p90', 'avg_ms', 'milp_avg',
                      'milp_max', 'milp_n', 'hold', 'ejects'):
        ...
        for name, _, _, _ in SIMS[1:]:
            ...
            print(f"  Fleet vs {name:<14}"
                  f"  cov={fm.final_cov - om.final_cov:+.1f}%"
                  f"  stair={fm.final_stair - om.final_stair:+.1f}%"
                  f"  found={fm.final_found - om.final_found:+d}"
                  f"  speed={om.avg_ms - fm.avg_ms:+.1f}ms/step (+ = Fleet faster)")

    n_seeds = len(seeds)
    avgs = {name: {k: v / n_seeds for k, v in accs[name].items()} for name in accs}
    ...
    if n_seeds > 1:
        ...
        for name, _, _, _ in SIMS:
            ...
            base = (f"  {name:<14}"
                    ...
                    f"  found={d['final_found']:.1f}/{EXT_N_SURVIVORS}"
    ...
    print(f"  Summary  ({steps} steps, {n_seeds} seed{'s' if n_seeds > 1 else ''},"
          f" {EXT_GRID_W}×{EXT_GRID_H} grid, {N_ROBOTS} robots, coastal nuclear scenario)")
    ...
    for name, _, _, _ in SIMS:
        ...
        print(f"  {name:<14}  {d['final_cov']:6.1f}  {d['final_stair']:7.1f}"
              ...
              f"  {d['trapped']:8.0f}  {d['avg_ms']:8.1f}  {ms}  {hold}  {ej}")
    print(f"{'=' * W}\n")

    # ── Save JSON ──────────────────────────────────────────────────────────────
    summary = {name: {k: round(float(v), 2) for k, v in avgs[name].items()} for name in avgs}
    summary['_config'] = {
        'scenario':    'coastal_nuclear_industrial_disaster',
        'grid':        f'{EXT_GRID_W}x{EXT_GRID_H}', 'cell_m': EXT_CELL_SIZE,
        'n_robots':    N_ROBOTS, 'robots': EXT_ROBOTS,
        'n_survivors': EXT_N_SURVIVORS,
        'survivor_split': '30% open / 40% industrial / 20% reactor / 10% waterfront',
        'reveal_r':    EXT_REVEAL_R, 'battery': EXT_MAX_BATTERY,
        'features': ['reactor_rad_gradient+leak', 'chemical_plume', 'tank/warehouse/river/reactor_fires',
                     'estuary_1intact_1collapsed_bridge', 'residential_terrace', 'rocky_hill_no_shadow', 'docks'],
        'steps':       steps, 'seeds': seeds,
    }
    with open(os.path.join(out_dir, 'summary_extreme.json'), 'w') as f:
        json.dump(summary, f, indent=2)

    # cache raw results so plotting can be re-run later without re-simulating
    try:
        import pickle
        with open(os.path.join(out_dir, 'results.pkl'), 'wb') as f:
            pickle.dump({'results_by_name': results_by_name, 'avgs': avgs,
                         'seeds': seeds, 'steps': steps}, f)
    except Exception as _e:
        print(f"  [warn: could not cache results.pkl: {_e}]")

    # ── Plots ──────────────────────────────────────────────────────────────────
    print("Generating plots...", end=' ', flush=True)
    saved = []
    SCEN = f'Coastal Nuclear Disaster ({EXT_GRID_W}×{EXT_GRID_H})'

    fig, ax = plt.subplots(figsize=(11, 5))
    _plot_metric(ax, results_by_name,
                 lambda r: [(s, v / r.total_survivors * 100) for s, v in r.found_ts],
                 'Survivors found (%)', f'Survivor Discovery — {SCEN}', steps, ylim=(0, 105))
    saved.append(_save(fig, out_dir, '01_survivors.png'))

    fig, ax = plt.subplots(figsize=(11, 5))
    _plot_metric(ax, results_by_name, lambda r: r.cov_ts,
                 'Coverage (%)', f'Map Coverage — {SCEN}', steps)
    saved.append(_save(fig, out_dir, '02_coverage.png'))

    fig, ax = plt.subplots(figsize=(11, 5))
    _plot_metric(ax, results_by_name, lambda r: r.stair_ts,
                 'Stair coverage (%)', f'Building Coverage — {SCEN}', steps, ylim=(0, 105))
    saved.append(_save(fig, out_dir, '03_stair_coverage.png'))

    fig, ax = plt.subplots(figsize=(11, 5))
    _plot_metric(ax, results_by_name,
                 lambda r: [(s, v / max(r.buildings_total, 1) * 100) for s, v in r.bldg_ts],
                 'Buildings entered (%)', f'Building Exploration — {SCEN}', steps, ylim=(0, 105))
    ...
    for name, _, col, _ in SIMS:
        ...
        if comp:
            axes[0].hist(comp, bins=max(1, min(8, len(comp))), alpha=0.5,
    ...
    axes[1].set_xlabel('Seed'); axes[1].set_ylabel('Steps'); axes[1].set_title('Completion Time per Seed')
    axes[1].legend(fontsize=7); axes[1].grid(alpha=0.3, axis='y')
    fig.suptitle(f'Completion Time — {SCEN}', fontsize=13)
    ...
    axes[0].set_xlabel('Seed'); axes[0].set_ylabel('Deaths'); axes[0].set_title('Robot Deaths per Seed')
    ...
    fig.suptitle(f'Safety Metrics — {SCEN}', fontsize=13)
    saved.append(_save(fig, out_dir, '06_safety.png'))

    fig, ax = plt.subplots(figsize=(11, 5))
    _plot_metric(ax, results_by_name, lambda r: r.step_ms_ts,
                 'ms / step', f'Computational Cost — {SCEN}', steps)
    txt = '\n'.join(f"{name}: {avgs[name]['avg_ms']:.1f} ms/step" for name, _, _, _ in SIMS)
    ...
    fig.suptitle(f'Coordination Redundancy — {SCEN}', fontsize=13)
    ...
    for name, _, col, _ in [s for s in SIMS if 'CARA' in s[0]]:
        for res in results_by_name[name]:
            if res.milp_ms_ts:
                axes[0].scatter([s for s, _ in res.milp_ms_ts], [v for _, v in res.milp_ms_ts],
                                color=col, s=20, alpha=0.6, label=f"{name} s={res.seed}")
    axes[0].set_xlabel('Timestep'); axes[0].set_ylabel('MILP solve time (ms)')
    axes[0].set_title('MILP Solve Times'); axes[0].legend(fontsize=7); axes[0].grid(alpha=0.3)
    cara_names = [s[0] for s in SIMS if 'CARA' in s[0]]
    cara_cols  = {s[0]: s[2] for s in SIMS if 'CARA' in s[0]}
    x2 = np.arange(n_seeds); w2 = 0.3
    for i, cname in enumerate(cara_names):
        vals = [r.hold for r in sorted(results_by_name[cname], key=lambda r: r.seed)]
        axes[1].bar(x2 + (i - 0.5) * w2, vals, w2, color=cara_cols[cname], label=cname, alpha=0.8)
    ...
    for bar, val in zip(bars, vals):
        ax_ms.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.05,
                   f'{val:.1f}', ha='center', va='bottom', fontsize=7)
    ...
    lines = ['Coastal Nuclear Disaster', '─' * 30,
             f"Grid:  {EXT_GRID_W}×{EXT_GRID_H} @ {EXT_CELL_SIZE}m",
             f"Seeds: {n_seeds}  |  Steps: {steps}",
             f"Robots: {N_ROBOTS}  |  Battery: {EXT_MAX_BATTERY}",
             f"Reveal R: {EXT_REVEAL_R}  |  Survivors: {EXT_N_SURVIVORS}",
             "Reactor · Chem plume · Fires", "Estuary · Hill · Docks", '',
             f"{'Sim':<14} {'Cov%':>5} {'Stair':>6} {'Found':>6} {'ms':>6}", '─' * 38]
    for name in names:
        d = avgs[name]
    ...
    fig.suptitle(f'Coastal Nuclear Industrial Disaster ({EXT_GRID_W}×{EXT_GRID_H}, {N_ROBOTS} robots)  —  '
