...
UNKNOWN_WATER_ADJ_PENALTY_LAND = 8.0   # big penalty -> avoid unless needed
UNKNOWN_NONWATER_PENALTY_BOAT  = 6.0   # boat dislikes unknown away from water

# ---------- Planner profile (A* cost shaping only) ----------
class PlannerProfile(Enum):
    TRAVEL_EXPLORE = 1
    MAX_SAFETY     = 2

PLANNER_PROFILE = PlannerProfile.TRAVEL_EXPLORE  # switch here

# Travel-explore (explore while travelling)
EXPLORE_UNK_PEN_SCOUT  = 0.25
EXPLORE_UNK_PEN_OTHER  = 0.70
EXPLORE_INFO_W_SCOUT   = 0.25   # reward per unknown neighbor revealed
EXPLORE_INFO_W_OTHER   = 0.10
EXPLORE_UNKNOWN_PRIOR  = 0.25   # assumed fraction of chunk risk in unknown
EXPLORE_SOFT_FRAC      = 0.90   # halo threshold for "unknown adjacency hazard"
EXPLORE_ALPHA_MULT     = 1.0
EXPLORE_BETA_MULT      = 1.0

# --- anti-conga / spread settings ---
USE_TRAFFIC_AVOIDANCE = True
TRAFFIC_LOOKAHEAD_K   = 25   # how many future steps contribute to traffic

TRAFFIC_W_EXPLORE = 0.25     # mild path spreading
TRAFFIC_W_SAFE    = 0.45     # stronger spreading in safety mode

# Optional: add repulsion in goal selection (helps when many frontiers exist)
USE_GOAL_REPULSION = True
GOAL_REPULSE_R     = 10      # cells
GOAL_REPULSE_W     = 0.25

# Max-safety
SAFE_UNK_PEN           = 2.50
SAFE_INFO_W            = 0.0
SAFE_UNKNOWN_PRIOR     = 0.70
SAFE_SOFT_FRAC         = 0.70
SAFE_ALPHA_MULT        = 1.6
SAFE_BETA_MULT         = 1.8
...
def traversable_code(tb: int, caps_mask: int) -> bool:
    ...
    if tb == T_WATER and (not has_water) and (not has_air):
        return False
    # bridge: allow LAND or WATER or AIR or STAIRS
    if tb == T_BRIDGE and (not has_land) and (not has_water) and (not has_air) and (not has_stairs):
        return False
...
class Cell:
    ...
    def cost(self, caps:set, terrain_override=None) -> float:
        t = terrain_override if terrain_override != None else self.true_terrain
        ...
        if t == T_OBS:
            return math.inf
        if t == T_STAIRS and Capability.STAIRS not in caps and Capability.AIR not in caps:
            return math.inf
        if t == T_WATER and Capability.WATER not in caps and Capability.AIR not in caps:
            return math.inf
        # BRIDGE should be like FREE land
        if t == T_BRIDGE:
            return 1.0
        if t == T_FREE and Capability.LAND not in caps and Capability.AIR not in caps:
            return math.inf
        return 1.0
    ...
    def _initialize_temperature(self):
        ...
        for x in range(self.w):
            for y in range(self.h):
                ...
                if self.grid[x][y].true_terrain in (T_WATER, T_BRIDGE):
                    temp = 5.0
...
class AStar:
    def __init__(self, world, start, goal, caps,
                ...
                zone_of_cell_fn,
                global_cov: float,
                unknown_step_penalty: float,
                info_gain_w: float,
                unknown_prior: float,
                alpha_mult: float,
                beta_mult: float,
                soft_frac: float,
                traffic_u16: np.ndarray,
                traffic_w: float,):
        self.world   = world
        self.start   = start
        self.goal    = goal
        ...
        self.unknown_step_penalty = float(unknown_step_penalty)
        self.info_gain_w = float(info_gain_w)
        self.unknown_prior = float(unknown_prior)
        self.alpha_mult = float(alpha_mult)
        self.beta_mult = float(beta_mult)
        self.soft_frac = float(soft_frac)
        self.traffic_u16 = traffic_u16
        self.traffic_w   = float(traffic_w)
    ...
    def astar_fast(
        ...
        soft_t = float(soft_frac) * temp_limit
        soft_r = float(soft_frac) * rad_limit
        ...
        while heap:
            ...
            for dx, dy in NBR4:
                ...
                if tb == T_BRIDGE and (not has_land) and (not has_air) and (not has_water) and (not has_stairs):
                    continue
                ...
                if tb == T_UNKNOWN:
                    ...
                    # for ddx, ddy in NBR4:
                    #     ax, ay = nx + ddx, ny + ddy
                    #     if 0 <= ax < W and 0 <= ay < H and terrain_belief_u8[ax, ay] == T_WATER:
                    #         water_adj = True
                    #         break

                    # # For boats, don't hard-block either; penalize unknown not adjacent to water
                    # unk_nonwater_boat = (boat_robot and (not water_adj))
                ...
                if radio_shadow_bool[nx, ny]:
                    ...
                    if z != None and (not relay_ok.get(z, False)):
                ...
                if tb != T_UNKNOWN:
                    ...
                    e = float(unknown_prior) * e_c

                    unk_pen = float(unknown_step_penalty)
                    # don't “freeze” late-game sweeping
                    if global_cov > 0.95:
                        unk_pen = min(unk_pen, 0.3)
                        
                # --- info gain reward (explore-while-travelling) ---
                info_bonus = 0.0
                w_info = float(info_gain_w)
                if w_info > 1e-9:
                    unk_n = 0
                    for ddx, ddy in NBR4:
                        ax, ay = nx + ddx, ny + ddy
                        if 0 <= ax < W and 0 <= ay < H and int(terrain_belief_u8[ax, ay]) == T_UNKNOWN:
                            unk_n += 1
                    info_bonus = w_info * float(unk_n)

                a_eff = float(alpha) * float(alpha_mult)
                b_eff = float(beta)  * float(beta_mult)



                step = base + unk_pen + a_eff * (e_c ** P) + b_eff * (e ** P) - info_bonus
                if traffic_w > 1e-9 and traffic_u16 is not None:
                    step += float(traffic_w) * float(traffic_u16[nx, ny])
                if step < MIN_STEP_COST:
                    step = MIN_STEP_COST
                ng = g + step
...
class Robot:
    def __init__(self, name, x, y, caps, world, sim,
        ...
        self.relay_hold_until = 0   # optional: minimum time to hold
    ...
    def set_goal(self, tgt):
        ...
        unk_pen, info_w, unk_prior, a_mult, b_mult, soft_frac, traffic_w = self.sim.planner_params(self)
        path = AStar(
            self.world,
            self.pos, tgt,
            self.caps,
            ...
            unknown_step_penalty=unk_pen,
            info_gain_w=info_w,
            unknown_prior=unk_prior,
            alpha_mult=a_mult,
            beta_mult=b_mult,
            soft_frac=soft_frac,
    ...
    def step(self):
        ...
        if (Capability.WATER in self.caps) and (Capability.AIR not in self.caps):
            if true_t not in (T_WATER, T_BRIDGE):
    ...
    def cell_traversable_under_belief(self, xy) -> bool:
        ...
        if self.sim.radio_shadow[x, y]:
            ...
            if (z != None) and (not self.sim.relay_ok.get(z, False)):
                return False
...
class FleetSim:
    def __init__(self):
        ...
        self.NO_CELL_COLLISION = True
        self.NO_SAME_LOCOMOTION_IN_ZONE = True ##Edit if needed
        ...
        self.ZONE_DONE = 0.98      # completion threshold (global)
        ...
        self._zone_stats_cache_key = None
                ...
                # Relay stabilization
        self.RELAY_MIN_HOLD = 60          # ticks: once you become relay, you must stay relay this long
        self.RELAY_COUNT_PENALTY = 2.5    # diminishing returns per relay already active (global or per zone)
        self.RELAY_SEP_RADIUS = 10        # cells: penalize relays that are too close
        self.RELAY_SEP_PENALTY = 6.0      # strength of separation penalty
        self.RELAY_MAX_PER_ZONE = 1       # hard cap (you already do something like this later; keep consistent
    ...
    def zone_frontiers(self, union, reachable, zone, robot=None):
        ...
        for x in xs:
            for y in ys:
                ...
                if is_boat:
                    # boats sweep water-edge: stand on water/bridge that reveals unknown
                    ...
                    if union[x, y] == T_UNKNOWN:
                        ...
                        # NEW: reveal-frontier (known cell adjacent to unknown)
                        if any(union[nx, ny] == T_UNKNOWN for (nx, ny) in self.world.neighbours((x, y))):
                            out.append((x, y))
    ...
    def assign_zones_cbba(self):
        ...
        for it in range(self.CBBA_ITERS):
            ...
            if self.NO_SAME_LOCOMOTION_IN_ZONE:
                winners = []
                used_types = set()
                for (nm, u) in claims_sorted:
                    rr = next((x for x in self.robots if x.name == nm), None)
                    if rr is None:
                        continue
                    t = robot_type(rr.name)  # "Legged"/"Drone"/"Boat"/"Rover"
                    if t in used_types:
                        continue
                    winners.append((nm, u))
                    used_types.add(t)
                    if len(winners) >= self.ZONE_CAPACITY:
                        break
            else:
                winners = claims_sorted[:self.ZONE_CAPACITY]

            winner_names = {nm for (nm, _u) in winners}
            losers = [c for c in claims_sorted if c[0] not in winner_names]
    ...
    def step(self):
        ...
        union = union_belief
        # ---- traffic map for anti-conga pathing (built once per tick) ----
        if USE_TRAFFIC_AVOIDANCE:
            self.traffic_u16.fill(0)
            for rr in self.robots:
                if (not rr.active) or (rr.path is None) or (len(rr.path) == 0):
                    continue
                # include current position a bit (discourage piling up)
                x0, y0 = rr.pos
                self.traffic_u16[x0, y0] = min(65535, int(self.traffic_u16[x0, y0]) + 2)

                for (px, py) in rr.path[:TRAFFIC_LOOKAHEAD_K]:
                    self.traffic_u16[px, py] = min(65535, int(self.traffic_u16[px, py]) + 1)

        global_cov = float(np.mean(union_belief != T_UNKNOWN))
        self.global_cov = global_cov
        ...
        zone_to_robots = {}
        for rr in self.robots:
            if rr.active and rr.task_zone != None:
        ...
        occupied = {rr.pos for rr in self.robots if rr.active and rr.battery > 0}
        ...
        def process_robot(r):
            ...
            if r.reachable_refresh > 0 and r.reachable_cache is not None:
                ...
                dq = deque([r.pos])
                reachable = {r.pos}
                caps_mask = caps_to_mask(r.caps)

                land_robot = (Capability.LAND in r.caps) and (Capability.AIR not in r.caps) and (Capability.WATER not in r.caps)
                boat_robot = (Capability.WATER in r.caps) and (Capability.AIR not in r.caps)
                ...
                while dq:
                    ...
                    for v in self.world.neighbours(u):
                        ...
                        x, y = v
                        tb = int(r.terrain_belief[x, y])

                        # Match A* UNKNOWN rules
                        if tb == T_UNKNOWN:
                            if land_robot:
                                water_adj = any(int(r.terrain_belief[nx, ny]) in (T_WATER, T_BRIDGE)
                                                for (nx, ny) in self.world.neighbours(v))
                                if water_adj:
                                    continue

                            if boat_robot:
                                water_adj = any(
                                    int(r.terrain_belief[nx, ny]) == T_WATER
                                    for (nx, ny) in self.world.neighbours(v)
                                )
                                if not water_adj:
                                    continue

                        if not traversable_code(tb, caps_mask):
                            continue

                        # Shadow feasibility must match A*
                        if self.radio_shadow[x, y]:
                            z = self.cell_to_zone(x, y)
                ...
                r.reachable_refresh = 12

            # --- RELAY EXIT CHECK ---
            if r.role == Role.RELAY and r.task_zone !=None:
                zx, zy = r.task_zone
                stats = self.compute_zone_stats_cached(zx, zy, self.union_belief, self.union_T, self.union_R)
                if self.should_exit_relay(r, r.task_zone, stats, reachable, union):
                    r.role = Role.SCAN
                    r.role_locked_until = self.timestep + 10
                    r.relay_anchor = None
                    r.relay_anchor_zone = None
            ...
            if r.role == Role.RELAY:
                if r.task_zone == None:
                    ...
                    r.path = []
                    return
                if getattr(r, "_relay_entered_zone", None) != r.task_zone:
                    self.enter_relay(r, r.task_zone)
                    r._relay_entered_zone = r.task_zone
                    
                if (r.relay_anchor == None) or (r.relay_anchor_zone != r.task_zone):
                    a = self.choose_zone_goal(r, r.task_zone, union, reachable)
                    r.relay_anchor = a
                    r.relay_anchor_zone = r.task_zone
                    r.relay_hold_until = self.timestep + 40

                if r.relay_anchor == None:
                    # no anchor available -> just stop safely
                    r.goal = None
                    r.path = []
                    r.reveal()
                    r.recompute_chunked()
                ...
                if r.pos != r.relay_anchor:
                    if r.goal != r.relay_anchor:
                        r.set_goal(r.relay_anchor)
                    if r.goal is not None and r.path:
                        next_cell = r.path[0]
                        if (not self.NO_CELL_COLLISION) or (next_cell not in occupied):
                            prev = r.pos
                            r.step()
                            if self.NO_CELL_COLLISION:
                        ...
                        # still update knowledge even if no path
                        r.reveal()
                        r.recompute_chunked()
                else:
                    # hold position
                ...
                return  # relays never run normal exploration logic
            ...
            if r.reachable_refresh > 0 and r.reachable_cache != None:
                reachable = r.reachable_cache
                r.reachable_refresh -= 1
            else:
                dq = deque([r.pos])
            ...
            if r.frontier_refresh > 0 and r.cached_frontiers != None:
            ...
            if r.task_zone is None:
                ...
                for z in r.assigned_zones:
                    ...
                    # Skip zones with no reachable frontier signal for THIS robot
                    z_front = self.zone_frontiers(union, reachable, z, robot=r)
                    if not z_front:
                        continue
            ...
            lease_active = (r.task_zone != None) and (self.timestep < r.zone_lease_until)
            ...
            if r.task_zone != None:
                ...
                elif r.task_zone != None:
                    # if zone has no reachable frontiers for this robot, drop it
                    z_front = self.zone_frontiers(union, reachable, r.task_zone, robot=r)
                    if not z_front:
                        # only drop if zone is basically done or truly hopeless
                        cov = self.zone_coverage(union_belief, r.task_zone)
                        if cov >= self.ZONE_DONE:
                            self.release_zone(r, reason="complete")
                        else:
                            # keep lease and just loiter/scan inside zone rather than abandoning
            ...
            ENDGAME = (remaining <= 3) or (global_cov > 0.97)
            ...
            def frontier_score(c):
                ...
                if r.name.startswith("Rover"):
                    base = dist - ALPHA * (e ** P)     # rover seeks risk
                else:
                    base = dist + ALPHA * (e ** P)     # others avoid risk

                # ---- multiplicative preferences (apply to base only) ----
                mult = 1.0

                # late-game "close pockets" preference: thin frontiers get cheaper
                if global_cov > 0.95:
                    unk_n = 0
                    for (nx, ny) in self.world.neighbours(c):
                ...
                if r.name.startswith("Boat") and (T_WATER in neigh_terr):
                    mult *= 0.6
                if r.name.startswith("Legged") and (T_STAIRS in neigh_terr):
                    mult *= 0.6
                if r.name.startswith("Drone") and ((T_STAIRS in neigh_terr) or (T_WATER in neigh_terr)):
                    mult *= 0.8

                # ---- crowding penalty (additive, not scaled by mult) ----
                crowd = 0.0
                for rr in self.robots:
                    if not rr.active:
                        continue
                    if rr.goal == c:
                        crowd += 1.0
                    elif rr.goal is not None and heuristic(rr.goal, c) < 6:
                        crowd += 0.5

                crowd_pen = 3.0 * crowd

                return base * mult + crowd_pen

            need_new_goal = (r.goal == None) or (r.pos == r.goal)
            failing = (r.stuck_steps > 10) or (r.goal == None)

            # Only change goal when allowed
            should_pick_new_goal = (r.goal == None) or (r.pos == r.goal) or (r.goal_commit == 0 and r.stuck_steps > 10)
            ...
            old_dist = heuristic(r.pos, r.goal) if r.goal != None else None

            if self.NO_CELL_COLLISION and r.path:
                next_cell = r.path[0]
                # block move if someone is already there (including a robot that moved earlier this tick)
                if next_cell in occupied:
                    # don't move; force replan next tick
                    r.path = []
                    r.goal_commit = 0
                    # optional: count as "stuck"
                    r.stuck_steps += 1
                else:
                    prev = r.pos
                    r.step()
                    # update occupied with actual move result
                    occupied.discard(prev)
                    occupied.add(r.pos)
            else:
                prev = r.pos
                ...
                if r.active and r.goal != None and old_dist != None:
        ...
        ## Upgrade to numpy arrays
        ## Prevent robots moving ontop of each other
    ...
    def lambda_breakdown(self, r, zone_stats, zones_assigned_count, *,
        ...
        # only value shadow if there's still info to gain
        # Only reward shadow where there is still exploration value.
        # If unknown_frac → 0, shadow should stop dominating the auction.
        shadow_bonus = 0.8 * sf * unknown_frac
    ...
    def choose_zone_goal(self, r, zone, union, reachable):
        ...
        def nearest_to_center(exclude=None, pool=None):
            if pool == None:
                pool = list(reachable)
            if exclude != None:
        ...
        # Frontiers (your existing logic)
        frontiers = self.zone_frontiers(union, reachable, zone, robot=r)

        # If no frontiers in-zone, don't crash—go to a safe anchor-ish point
        if not frontiers:
            # prefer in-zone if possible
            pool = [p for p in reachable if in_zone(p)]
            tgt = nearest_to_center(pool=pool) if pool else nearest_to_center()
            return tgt

        # LOITER: drift around zone center (but don't stand still)
        if r.role == Role.LOITER:
            pool = [p for p in reachable if in_zone(p)]
            tgt = nearest_to_center(exclude=r.pos, pool=pool) if pool else nearest_to_center(exclude=r.pos)
            return tgt

        # RELAY: pick an "anchor" outside shadow, preferably touching shadow boundary
        if r.role == Role.RELAY:

            def is_outside_shadow(p):
                x, y = p
                return not self.radio_shadow[x, y]

            def touches_shadow(p):
                x, y = p
                for (nx, ny) in self.world.neighbours((x, y)):
                    if self.radio_shadow[nx, ny]:
                        return True
                return False

            # 1) Strict anchors: in-zone, outside shadow, touching shadow
            anchors = [p for p in reachable if in_zone(p) and is_outside_shadow(p) and touches_shadow(p)]

            # If none, relax to: in-zone + outside shadow
            if not anchors:
                anchors = [p for p in reachable if in_zone(p) and is_outside_shadow(p)]

            # If still none, relax to: anywhere outside shadow
            if not anchors:
                anchors = [p for p in reachable if is_outside_shadow(p)]

            # If *still* none, nowhere safe
            if not anchors:
                return None
            ...
            def relay_score(p):
                ...
                # Prefer being somewhat near border to help neighbor zones too
                dist_to_border = min(x - x0, x1 - x, y - y0, y1 - y)
                dist_to_border = max(0, dist_to_border)  # safety clamp

                border_bonus = 2.0 / (1.0 + dist_to_border)  # safe now (>= 1)

                # Prefer staying near zone center
                center_cost = 0.15 * heuristic(p, (cx, cy))

                # Prefer points that touch more shadow (better relay service)
                shadow_touch = 0
                for (nx, ny) in self.world.neighbours((x, y)):
                    if self.radio_shadow[nx, ny]:
                        shadow_touch += 1
                ...
                return center_cost - 1.5 * border_bonus - 0.8 * shadow_touch

            return min(anchors, key=relay_score)

        # SCOUT: pick frontier with high info gain (more unknown neighbors)
        ...
        # Default SCAN: distance + chunk risk (your existing idea)
        def scan_score(c):
            dist = heuristic(r.pos, c)
    ...
    def role_utility(self, r: Robot, role: Role, zone, role_counts, stats):
        ...
        global_relays = sum(1 for rr in self.robots if rr.active and rr.role == Role.RELAY)
        zone_relays = role_counts.get(Role.RELAY, 0)
        ...
        if role == Role.RELAY:
            # diminishing return: each existing relay makes new relays less valuable
            U_rule -= self.RELAY_COUNT_PENALTY * global_relays
            # optional: extra pressure locally
            U_rule -= 2.0 * zone_relays
            # separation penalty: discourage relays parking on top of each other
            rx, ry = r.pos
            sep_pen = 0.0
            R = self.RELAY_SEP_RADIUS

            for rr in self.robots:
                if rr is r or (not rr.active) or rr.role != Role.RELAY:
                    continue
                # only compare against relays in same or neighboring zones (keeps it local)
                if zone is not None:
                    z_rr = self.cell_to_zone(rr.pos[0], rr.pos[1])
                    if z_rr is None:
                        continue
                    if z_rr != zone and z_rr not in self.zone_neighbors4(zone):
                        continue
                ...
                if d2 < R*R:
                    ...
                    sep_pen += (1.0 - math.sqrt(d2)/max(1e-6, R))

            U_rule -= self.RELAY_SEP_PENALTY * sep_pen
    ...
    def decide_roles_in_zone(self, zone, robots_in_zone, stats):
        ...
        for _ in range(3):  # 3 rounds best-response
            ...
            for r in robots_in_zone:
                ...
                if old_role != Role.RELAY and r.role == Role.RELAY:
                    r.relay_hold_until = self.timestep + self.RELAY_MIN_HOLD
                    r.role_locked_until = self.timestep + self.RELAY_MIN_HOLD  # prevents immediate flip
                role_counts[r.role] = role_counts.get(r.role, 0) + 1

                # lock to prevent oscillation
                r.role_locked_until = self.timestep + 15
        # Hard cap: at most 1 relay per zone (prevents relay dogpiles)
        relays = [rr for rr in robots_in_zone if rr.active and rr.role == Role.RELAY]
        if len(relays) > self.RELAY_MAX_PER_ZONE:
            relays_sorted = sorted(relays, key=lambda rr: (getattr(rr, "zone_frontier_signal", 0.0), rr.battery), reverse=True)
            keep = relays_sorted[0]
            for rr in relays_sorted[1:]:
                # only demote if not in mandatory hold (avoid thrash)
                if self.timestep >= getattr(rr, "relay_hold_until", 0):
                    rr.role = Role.SCAN
                    rr.role_locked_until = self.timestep + 10
    ...
    def enter_relay(self, r, zone):
        """When a robot becomes a relay, it should stop hoarding exploration zones."""
        if zone == None:
            return

        # Release all other zones it was holding/assigned
        for z in list(getattr(r, "assigned_zones", [])):
            if z != zone:
                self.release_zone(r, reason="entered_relay")

        # Force it to only care about its relay zone
        r.bundle = [zone]
        r.assigned_zones = [zone]
        r.task_zone = zone
        r.zone_lease_until = self.timestep + self.LEASE_T

        # Reset relay anchor so it re-picks cleanly
        r.relay_anchor = None
        r.relay_anchor_zone = None
        r.relay_hold_until = 0


    def should_exit_relay(self, r, zone, stats, reachable, union):
        """Decide if a relay should stop relaying and return to exploration."""
        # Must hold relay role for minimum duration
        if self.timestep < getattr(r, "relay_hold_until", 0):
            return False
        
        if zone == None:
            return True

        # If zone mostly explored, relaying is likely not helpful anymore
        cov = self.zone_coverage(self.union_belief, zone)
        if cov >= 0.985:
            return True

        # If there is barely any shadow in this zone, don't relay
        sf = max(stats.get("shadow_frac_total", 0.0), stats.get("shadow_frac", 0.0))
        if sf < 0.03:
            return True

        # If there are no shadow-adjacent anchor candidates, don't relay
        anchors = []
        for (x, y) in reachable:
            if self.radio_shadow[x, y]:
                continue
            if any(self.radio_shadow[nx, ny] for (nx, ny) in self.world.neighbours((x, y))):
                anchors.append((x, y))
        if not anchors:
            return True

        # If we already have another relay in this zone, don't be redundant
        other_relay = any(
            rr.active and rr.role == Role.RELAY and rr != r and rr.task_zone == zone
            for rr in self.robots
        )
        if other_relay:
            return True
    ...
    def planner_params(self, r: "Robot"):
        """Return per-robot A* shaping parameters based on global planner profile + role."""
        if self.planner_profile == PlannerProfile.TRAVEL_EXPLORE:
            traffic_w = TRAFFIC_W_EXPLORE

            if getattr(r, "role", None) == Role.SCOUT:
                return (EXPLORE_UNK_PEN_SCOUT, EXPLORE_INFO_W_SCOUT,
                        EXPLORE_UNKNOWN_PRIOR, EXPLORE_ALPHA_MULT, EXPLORE_BETA_MULT,
                        EXPLORE_SOFT_FRAC, traffic_w)

            return (EXPLORE_UNK_PEN_OTHER, EXPLORE_INFO_W_OTHER,
                    EXPLORE_UNKNOWN_PRIOR, EXPLORE_ALPHA_MULT, EXPLORE_BETA_MULT,
                    EXPLORE_SOFT_FRAC, traffic_w)

        # --- SAFE / default profile ---
        traffic_w = TRAFFIC_W_SAFE

        if getattr(r, "role", None) == Role.SCOUT:
            return (SAFE_UNK_PEN_SCOUT, SAFE_INFO_W_SCOUT,
                    SAFE_UNKNOWN_PRIOR, SAFE_ALPHA_MULT, SAFE_BETA_MULT,
                    SAFE_SOFT_FRAC, traffic_w)

        return (SAFE_UNK_PEN_OTHER, SAFE_INFO_W_OTHER,
                SAFE_UNKNOWN_PRIOR, SAFE_ALPHA_MULT, SAFE_BETA_MULT,
                SAFE_SOFT_FRAC, traffic_w)
...
def build_grid_surface(sim, show_map, show_survivors, show_heat, show_rad, show_risk,
    ...
    # -----------------------------
    # Precompute risk map if needed
    # -----------------------------
    if show_risk:
        chunk_W = (world.w + CHUNK_SIZE - 1) // CHUNK_SIZE
        chunk_H = (world.h + CHUNK_SIZE - 1) // CHUNK_SIZE

        chunk_max_T = np.zeros((chunk_W, chunk_H), dtype=float)
        chunk_max_R = np.zeros((chunk_W, chunk_H), dtype=float)
        chunk_known = np.zeros((chunk_W, chunk_H), dtype=np.uint8)
        ...
        for cx in range(chunk_W):
            for cy in range(chunk_H):
                ...
                for x in xs:
                    for y in ys:
                        if show_map:
                            # TRUE global hazards when map is shown
                            valsT.append(float(world.grid[x][y].temperature))
                            valsR.append(float(world.grid[x][y].radiation))
                        else:
                            # Fog-of-war: only use known cells
                            if union_belief[x, y] != T_UNKNOWN:
                                t = union_T[x, y]
                                r = union_R[x, y]
        ...
        # mix hazards into one risk value (tune multipliers if you want)
    ...
    for x in range(world.w):
        for y in range(world.h):
            ...
            # ---- BASE TERRAIN (map vs fog) ----
            if show_map:
                terr = world.grid[x][y].true_terrain
                clr = TERRAIN_COLOUR_CODE[int(terr)]
                visible = True
            else:
                terr = union_belief[x, y]
                clr = TERRAIN_COLOUR_CODE[int(terr)]
                visible = (terr != T_UNKNOWN)

            # ---- OVERLAYS ----
            if show_risk:
                # show_map: everywhere; no show_map: only visible cells
                if show_map or visible:
                    cx, cy = x // CHUNK_SIZE, y // CHUNK_SIZE
                    if chunk_known[cx, cy]:
                        risk_norm = min(max(risk_map[cx, cy] / max_risk, 0.0), 1.0)
                        clr = (int(255 * risk_norm), int(255 * (1.0 - risk_norm)), 0)
                    else:
                        # unknown chunk (fog mode) -> grey
                        clr = (180, 180, 180)

            elif show_heat:
                if show_map or visible:
                    tt = world.grid[x][y].temperature if show_map else union_T[x, y]
                    tt = 0.0 if np.isnan(tt) else float(tt)
                    t_norm = min(max(tt / 200.0, 0.0), 1.0)
                    clr = (int(255 * t_norm), 0, int(255 * (1.0 - t_norm)))

            elif show_rad:
                if show_map or visible:
                    rr = world.grid[x][y].radiation if show_map else union_R[x, y]
                    rr = 0.0 if np.isnan(rr) else float(rr)
                    v = min(rr / 100.0, 1.0)
                    clr = (0, int(255 * v), 0)

            # ---- SURVIVORS ON TOP (even in fog) ----
...
def draw_grid(scr, sim, show_zones):
    ...
    for zx in range(sim.zone_nx):
        for zy in range(sim.zone_ny):
            ...
            owner_names = list(t.owners) if (t != None and hasattr(t, "owners")) else []
...
def gui_loop():
    ...
    while True:
        ...
        if (grid_surface == None) or (vk != last_view_key) or (union_matters and union_changed):
        ...
        if show_shadow:
            if shadow_dirty or shadow_surface == None:
                shadow_surface = build_shadow_surface(sim)
                shadow_dirty = False
