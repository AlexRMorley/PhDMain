...
class CARABot:
    __slots__ = (
        ...
        'is_relay',
        'relay_cluster',
        '_chunked_buf',      # pre-allocated buffers for _recompute_chunked
    ...
    def __init__(self, name, x, y, caps, caps_mask, world, sim,
        ...
        self._chunked_buf=(np.zeros((M.GRID_W,M.GRID_H),dtype=np.float32),
                           np.zeros((M.GRID_W,M.GRID_H),dtype=np.float32))
    ...
    def _scan(self):
        ...
        if self._scan_dirty:
            self._recompute_chunked(); self._scan_dirty=False

    def _recompute_chunked(self):
        """Update max-pooled hazard risk map from known temp/rad beliefs."""
        M=self.sim.M
    ...
    def _plan_to_ignoring_shadow(self, goal):
        ...
        Hazard avoidance is still active: even on emergency exit, a robot
        should not route through cells it knows are above its hazard limit."""
        M=self.sim.M
        _zt=np.zeros((M.GRID_W,M.GRID_H),dtype=np.uint16)
        _no_shadow=np.zeros((M.GRID_W,M.GRID_H),dtype=bool)
        path=M.AStar.search(
            ...
            alpha_mult=1.0, beta_mult=1.0, soft_frac=0.85,
    ...
    def tick(self, occupied):
        ...
        # ── Hazard replan-on-sense (fork) ──
        # If the committed path now crosses a cell this robot has LEARNED is
        # lethal, drop the path so the role logic below replans around it.
        # Paths are planned through then-unknown cells (A* treats unknown as
        # safe); as the robot scans it learns some are lethal, but without this
        # it keeps walking the old path straight onto them — the dominant CARA
        # death mode.  A*'s hard hazard-block then routes the new path clear.
        if self.path:
            tl=self.temp_limit; rl=self.rad_limit
            for (px,py) in self.path:
                if self.known_mask[px,py] and (
                        self.temp_belief[px,py] > tl or self.rad_belief[px,py] > rl):
...
def make_cara_sim(gnf_module, M_module, use_exec_layer=True):
    ...
    class CARASim(GNFSim):
        ...
        def _solve_milp(self):
            ...
            # ── Per-zone KNOWN hazard exposure (fork: make the MILP hazard-aware) ──
            # Previously the allocation objective was pure coverage, so it would
            # dispatch fragile robots (Legged/Drone) into zones it had already
            # sensed to be lethal — hazard-aware A* then could not save a robot
            # sent to explore a furnace.  Use the shared belief's MAX known temp/
            # rad in each zone (unknown cells -> 0 via nan_to_num, so they don't
            # inflate it).  Boats/Rovers carry ~9999 limits -> exposure 0 -> immune.
            uT=getattr(self,'union_T',None); uR=getattr(self,'union_R',None)
            zmaxT={}; zmaxR={}
            for z in zones:
                zx,zy=z; x0=zx*self.zone_w_cells; x1=min(x0+self.zone_w_cells,M.GRID_W)
                y0=zy*self.zone_h_cells; y1=min(y0+self.zone_h_cells,M.GRID_H)
                zmaxT[z]=float(np.nan_to_num(uT[x0:x1,y0:y1]).max()) if uT is not None else 0.0
                zmaxR[z]=float(np.nan_to_num(uR[x0:x1,y0:y1]).max()) if uR is not None else 0.0
            HAZ_SOFT=0.85   # start penalising at 85% of the robot's own limit
            HAZ_W=8.0       # weight — dominates the (<=1.0) coverage term
            forbid_pairs=[] # (ri,zi) zones already known lethal for that robot
            ...
            for ri,r in enumerate(robots):
                ...
                for zi,z in enumerate(zones):
                    ...
                    # Hazard exposure for THIS robot relative to ITS OWN limits.
                    ex=(max(0.0, zmaxT[z]/max(1e-6,r.temp_limit)-HAZ_SOFT)
                        + max(0.0, zmaxR[z]/max(1e-6,r.rad_limit)-HAZ_SOFT))
                    c_vec[ri*Z+zi]=-(zuf[z]*cap_ok*shd_factor)+HAZ_W*ex
                    # Hard-forbid a zone already known to hold a lethal cell for r.
                    # (A* still avoids the specific cells; this stops the allocator
                    #  sending r there at all.  Worst case r gets no zone and idles,
                    #  which is strictly better than being dispatched to its death.)
                    if zmaxT[z]>r.temp_limit or zmaxR[z]>r.rad_limit:
                        forbid_pairs.append((ri,zi))
                for ci in range(C):
            ...
            # Hazard hard-forbid: x[ri,zi] <= 0 for zones already known lethal to r.
            for (ri, zi) in forbid_pairs:
                rows.append(n_con); cols.append(ri*Z+zi); data.append(1.0)
                b_lo_l.append(-np.inf); b_up_l.append(0.0); n_con+=1
