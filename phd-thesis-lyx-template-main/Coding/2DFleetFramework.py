...
class AStar:
    ...
    def search(self):
            ...
            # NOTE:
            # self.weights is a 2-vector (wT, wR).
            # For avoiders: positive weights -> risk increases cost.
            # For seekers (Rover): negative weights -> risk decreases cost.
            #
            # We normalize temp/rad by THIS robot's limits before applying weights.
            # We apply the same "lambda" idea at both chunk level and cell level.
            ...
            while open_set:
                ...
                for nbr in self.world.neighbours(current):
                    ...
                    # terrain bonuses (based on BELIEF)
                    if tb is not Terrain.UNKNOWN:
                    ...
                    # -----------------------------
                    # λ risk term (chunk + cell)
                    # -----------------------------
                    cx, cy = nbr[0] // CHUNK_SIZE, nbr[1] // CHUNK_SIZE
                    lamT_c = self.chunked[0, cx, cy]
                    lamR_c = self.chunked[1, cx, cy]

                    # normalize by THIS robot limits
                    eT_c = lamT_c / max(1e-6, self.temp_limit)
                    eR_c = lamR_c / max(1e-6, self.rad_limit)

                    # weighted lambda signal (can be negative for rover)
                    wT = float(self.weights[0])
                    wR = float(self.weights[1])
                    lam_sig_c = (wT * eT_c) + (wR * eR_c)

                    # per-cell exposure (only if known)
                    lam_sig_cell = 0.0
                    if tb is not Terrain.UNKNOWN:
                        t = self.temp_belief[nbr[0], nbr[1]]
                        r = self.rad_belief[nbr[0], nbr[1]]
                        t_val = 0.0 if np.isnan(t) else float(t)
                        r_val = 0.0 if np.isnan(r) else float(r)
                        eT = t_val / max(1e-6, self.temp_limit)
                        eR = r_val / max(1e-6, self.rad_limit)
                        lam_sig_cell = (wT * eT) + (wR * eR)

                    # shape: mild when safe, steep near limit
                    # uses global P in your file
                    p = P

                    # convert lambda signal to cost:
                    # - positive lam_sig => penalty (avoid)
                    # - negative lam_sig => "bonus" (seek) by subtracting
                    chunk_term = self.alpha * (abs(lam_sig_c) ** p)
                    if lam_sig_c < 0.0:
                        chunk_term = -chunk_term

                    cell_term = BETA * (abs(lam_sig_cell) ** p)
                    if lam_sig_cell < 0.0:
                        cell_term = -cell_term

                    # total step cost:
                    # ensure strictly positive (A* requires non-negative costs)
                    step_cost = base + chunk_term + cell_term
                    step_cost = max(0.01, step_cost)
...
class FleetSim:
    ...
    def step(self):
        ...
        for r in self.robots:
            ...
            # Decide whether we are allowed to change goal now
            should_pick_new_goal = need_new_goal or (r.goal_commit == 0 and failing)

            if should_pick_new_goal:
                # --- B4: goal selection MUST come from assigned task zone ---
                if r.task_zone is not None:
                    tgt = self.choose_zone_goal(r, r.task_zone, union, reachable)

                    # if no sensible goal exists, release zone and continue
                    if tgt is None:
                        self.release_zone(r, reason="no_frontier")
                        continue

                    # if zone is basically done, release
                    ...
                    # fallback: global frontier exploration if no zone assigned
                    if not r.cached_frontiers:
                        r.goal = None
                        r.path = []
                        continue

                    # simple close frontier fallback
                    tgt = min(r.cached_frontiers, key=lambda c: heuristic(r.pos, c))
                    r.set_goal(tgt)
                    r.stuck_steps = 0

            # Now move one step along the current goal/path
    ...
    def lambda_breakdown(self, r, zone_stats, zones_assigned_count, *,
            ...
            # -----------------------------
            # Risk affinity (DEFENSIBLE)
            # -----------------------------
            # Normalize by robot limits, then apply weights.
            ...
            # If lam_sig > 0 => avoiders will have positive penalty signal
            # If lam_sig < 0 => seekers (rover) will see negative signal
            # We convert signal into affinity for utility:
            #  - avoiders: higher risk => LOWER utility
            #  - seekers:  higher risk => HIGHER utility
            #
            # Using -(abs(lam_sig)**P) and +(...) for rover is easiest to interpret.
            if r.name in ("Legged", "Drone"):
                risk_affinity = -(abs(lam_sig) ** P)
            elif r.name == "Rover":
                risk_affinity = +(abs(lam_sig) ** P)
            else:
                risk_affinity = 0.0  # Boat neutral to heat/rad
    ...
    if show_zones:
        for zx in range(sim.zone_nx):
            for zy in range(sim.zone_ny):
                ...
                task = sim.zone_tasks.get(zone)
                status = task.status if task else "free"
                owner_name = task.owner if task else None

                # Decide border style by status
                if status == "blacklisted":
                    # blacklisted: red-ish border
                    border_color = (200, 0, 200)
                    width = 2
                elif status == "held" and owner_name is not None and sim.timestep < task.expires_at:
                    # held: owner color
                    border_color = ROBOT_COLOUR.get(owner_name, (120, 120, 120))
                    width = 3
                ...
                pygame.draw.rect(scr, border_color, rect, width)

        # highlight each robot's CURRENT TASK ZONE thicker (B2/B4 clarity)
        for r in sim.robots:
            if not r.active:
                continue
            if r.task_zone is None:
            ...
            pygame.draw.rect(scr, ROBOT_COLOUR[r.name], rect, 5)

        # keep your old goal-zone highlight too (optional, but useful)
        for r in sim.robots:
            if r.goal is None:
                continue
            gx, gy = r.goal
            ...
            pygame.draw.rect(scr, ROBOT_COLOUR[r.name], rect, 2)
